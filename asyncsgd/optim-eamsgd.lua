   ...
   local function localupdate()
      if lr ~= 0 then
		 if mom > 0 then
			...
			state.vt:mul(mom) -- vt = mom * vt
			w:add(state.vt)
		 end
		 fx,dfdx = opfunc(w) -- forward && backward
		 ...
		 state.pversion = state.pversion + 1 -- local # of iterations
      end
   end

   -- only param client do local training
   if (pc and su>0 and mva>0) then
      if (state.pversion%su == 0) then
--		  print('pc #' .. rank .. ' sync with param server')
